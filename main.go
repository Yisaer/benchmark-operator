...
func main() {
	...
	if err = (&controllers.TpccBenchmarkReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("TpccBenchmark"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "TpccBenchmark")
		os.Exit(1)
	}
	if err = (&controllers.DataBaseBenchmarkPrepareReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("DataBaseBenchmarkPrepare"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
