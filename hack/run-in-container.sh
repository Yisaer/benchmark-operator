...
DOCKER_LIB_VOLUME=${DOCKER_LIB_VOLUME:-test-operator-docker-lib}
DOCKER_GRAPH_VOLUME=${DOCKER_GRAPH_VOLUME:-test-operator-docker-graph}
DOCKER_GO_VOLUME=${DOCKER_GO_VOLUME:-test-operator-go}
NAME=${NAME:-test-operator-dev}
...
Environments:
    ...
    DOCKER_LIB_VOLUME   the name of docker lib volume, defaults: test-operator-docker-lib
    DOCKER_GRAPH_VOLUME the name of docker graph volume, defaults: test-operator-docker-graph
    DOCKER_GO_VOLUME    the name of go cache volume, defaults: test-operator-go
...
docker run ${docker_args[@]} \
    -p 80 \
    -v $ROOT:/go/src/test-operator \
    -w /go/src/test-operator \
